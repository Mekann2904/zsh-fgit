...
* ベースディレクトリは `"$HOME"` が既定
* 検索深さは `FGIT_DEPTH` で指定（未設定なら 5、1以上の整数）
...
* `rg` があれば利用し、なければ `grep -rl` にフォールバックする（リポジトリ探索の `fd` とは独立）
...
* **`rg` がない場合の Grep モード**は `grep -rl` を使うため、環境によっては遅い（特にベースディレクトリが広い場合）。高速化したいなら `rg` を推奨する。
